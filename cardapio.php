...
// Filtro de busca por nome (client-side seria melhor, mas mantemos PHP puro)
if ($busca !== '') {
...
<div class="top">
    <a class="back" href="mesa.php?id=<?= $mesa_id ?>">←</a>
    <h1>Cardápio</h1>
    <small>Mesa <?= $mesa_id ?></small>
</div>
...
<div class="cats">
    <a
        href="?mesa_id=<?= $mesa_id ?>&comanda_id=<?= $comanda_id ?>"
        class="cat <?= !$cat_id ? 'active' : '' ?>"
    >Tudo</a>
    <?php foreach ($categorias as $cat): ?>
        <a
            href="?mesa_id=<?= $mesa_id ?>&comanda_id=<?= $comanda_id ?>&cat_id=<?= $cat['id'] ?>"
    ...
    <?php else: ?>
        <?php foreach ($produtos as $p): ?>
            <div class="card">
                ...
                <a
                    href="adicionar_item.php?mesa_id=<?= $mesa_id ?>&comanda_id=<?= $comanda_id ?>&produto_id=<?= $p['id'] ?>"
                    class="btn-add"
                >+ Adicionar pedido</a>
            </div>
        <?php endforeach; ?>
